...
const unavailableTitle = (item: UserNotification) => {
  ...
  if (item.type === 'password_update_incomplete') return '密码修改未完成'
  return '关联内容暂不可用'
}
...
const unavailableDetail = (item: UserNotification) => {
  ...
  if (item.type === 'vocechat_credentials') return '你的 VoceChat 账号信息可能已发生变化，这可能导致本站部分功能暂时不可用。请前往后台“系统推送”，重新填写并校验 VoceChat 邮箱和密码；验证成功后，此通知将自动消失。'
