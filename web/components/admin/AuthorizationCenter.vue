<template>
  <section class="admin-setting-block space-y-4">
    ...
    <div class="grid gap-3 md:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <div v-if="selectedID" class="space-y-4"><div v-for="group in groups" :key="group.module" class="rounded-md border p-3 dark:border-gray-700"><div class="mb-2 flex items-center justify-between gap-3"><h3 class="font-medium">{{ group.module }}</h3><UCheckbox :model-value="group.grantable.every(item => selectedCapabilities.has(item.capability))" :indeterminate="group.grantable.some(item => selectedCapabilities.has(item.capability)) && !group.grantable.every(item => selectedCapabilities.has(item.capability))" label="全选此模块" @update:model-value="toggleGroup(group, $event === true)" /></div><div class="grid gap-2 sm:grid-cols-2"><UCheckbox v-for="item in group.items" :key="item.capability" :model-value="selectedCapabilities.has(item.capability)" :disabled="!item.grantable" :label="item.label + (!item.grantable ? '（仅站长）' : '')" @update:model-value="toggle(item.capability, $event === true)" /></div></div><div class="flex items-center gap-3"><UButton :loading="saving" :disabled="!dirty" color="primary" @click="save">保存授权</UButton><span v-if="dirty" class="text-sm text-amber-600">存在未保存变更</span><span v-if="message" class="text-sm" :class="messageError ? 'text-red-600' : 'text-green-600'">{{ message }}</span></div></div>
