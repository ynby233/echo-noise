...
const applySettings = ({ frontendSettings }: any) => { enabled.value = booleanSetting(frontendSettings.socialLinksEnabled); links.value = Array.isArray(frontendSettings.socialLinks) ? frontendSettings.socialLinks.map((item: any) => ({ name: String(item?.name || ''), url: String(item?.url || ''), icon: String(item?.icon || '') })) : [] }
const save = async () => {
  if (!ready.value || loading.value) return
  try { const cleaned = links.value.map(item => ({ name: item.name.trim(), url: item.url.trim(), icon: item.icon.trim() })).filter(item => item.url); await saveFrontendSettings({ socialLinksEnabled: enabled.value, socialLinks: cleaned }); links.value = cleaned; await saved(); toast.add({ title: '成功', description: '社交链接已更新', color: 'green' }) }
  catch (error: any) { toast.add({ title: '保存失败', description: error.message, color: 'red' }) }
}
const { ready, loading, error, load, saved } = useConfigDraft('social', reactive({ enabled, links }), loadFrontendSettings, applySettings)
</script>
