<template>
  <section id="site-configs-section" :class="adminShellCardClass">
    ...
    <input ref="uploadInput" type="file" accept="image/*" multiple class="hidden" @change="uploadBackgrounds" />
...
const applySettings = (settings: Record<string, any>) => { for (const key of Object.keys(defaults) as Key[]) form[key] = key === 'backgrounds' ? (Array.isArray(settings.backgrounds) ? settings.backgrounds.map(normalizeBackground).filter((item: Background) => item.url) : []) : String(settings[key] ?? defaults[key]) }
const save = async (key: Key) => { if (!ready.value || loading.value) return; try { const value = key === 'backgrounds' ? form.backgrounds.map(normalizeBackground).filter((item: Background) => item.url.trim()) : form[key]; await saveFrontendSettings({ [key]: value }); useToast().add({ title: '成功', description: `${labels[key]}已更新`, color: 'green' }); await saved([key]) } catch (error: any) { useToast().add({ title: '失败', description: error?.message || '保存失败', color: 'red' }) } }
...
const { ready, loading, error, load, saved } = useConfigDraft('site-info', form, async () => (await loadFrontendSettings()).frontendSettings, applySettings)
</script>
