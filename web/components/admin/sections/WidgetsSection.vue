...
const load = async () => {
  const { frontendSettings } = await loadFrontendSettings(); apply(personal, frontendSettings)
  if (isPrimaryAdmin.value) { const response = await fetch(`${baseApi}/admin/guest-widget-preferences`, { credentials: 'include' }); const body = await response.json(); if (response.ok && body?.code === 1) apply(guest, body.data) }
}
const save = async (scope: 'personal' | 'guest') => {
  ...
  try {
    ...
    await load(); window.dispatchEvent(new Event('frontend-config-updated')); toast.add({ title: '成功', description: scope === 'personal' ? '我的小组件已保存' : '访客默认已保存', color: 'green' })
  } catch (error: any) { toast.add({ title: '失败', description: error.message || '保存失败，已恢复服务器当前值', color: 'red' }); await load() }
