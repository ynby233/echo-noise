...
assert.match(panel, /can\('audit\.view'\)[\s\S]*?key: 'admin-audit'/)
...
assert.match(panel, /security:\s*\(\)\s*=>\s*import\('\.\/SecuritySection\.vue'\)/)
assert.match(panel, /'access-logs':\s*\(\)\s*=>\s*import\('\.\/AccessLogsSection\.vue'\)/)
assert.match(panel, /'site-visits':\s*\(\)\s*=>\s*import\('\.\/SiteVisitsSection\.vue'\)/)
assert.match(panel, /'login-audits':\s*\(\)\s*=>\s*import\('\.\/LoginAuditsSection\.vue'\)/)
assert.match(panel, /notify:\s*\(\)\s*=>\s*import\('\.\/NotificationConfigSection\.vue'\)/)
...
assert.match(center, /music: '音乐配置'/, 'music authorization must have a clear module label')
assert.match(center, /email: '邮件配置'/, 'email authorization must use the renamed module label')
assert.match(center, /'notes', 'comments'/, 'the two tall content-management modules must remain last in the authorization grid')
