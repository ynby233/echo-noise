...
assert.match(carousel, /--promo-text-color/, '每条广告的文字颜色应传入渲染样式')
assert.match(carousel, /class="promo-frame"/, '广告容器应使用不会命中通用广告过滤规则的中性样式钩子')
assert.doesNotMatch(carousel, /(?:class="ad-wrap"|\.ad-wrap\b)/, '广告容器不得使用会被 EasyList 和 AdGuard Base 全局隐藏的 ad-wrap')
