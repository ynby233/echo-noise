...
assert.match(statusPanel, /v-if="canManageNotificationState"[\s\S]*?<UToggle v-model="notifyEnabled"[\s\S]*?@click="saveEnabled"/, 'the global notification switch and save action must be absent unless the backend write capabilities are present')
