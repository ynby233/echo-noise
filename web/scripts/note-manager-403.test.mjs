...
try {
  ...
  await load()
  handleForbidden.reset()
  await handleForbidden()
  await handleForbidden()
  assert.equal(requests, 1, 'a continuous 403 load must not issue a recursive request')
  assert.equal(refreshes, 2, 'a new permission generation must refresh capabilities again')
  assert.equal(toasts, 2, 'each permission generation must show one toast')
