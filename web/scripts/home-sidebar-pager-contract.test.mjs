...
assert.match(sidebarPager, /\.home-sidebar-pager__main\s*\{[^}]*grid-template-columns:\s*28px minmax\(0, 1fr\) 28px;/, '页首和页尾按钮必须与两侧翻页按钮使用相同的正方形列宽')
assert.match(sidebarPager, /\.home-sidebar-pager__scroll\s*\{[^}]*width:\s*28px;[^}]*height:\s*28px;[^}]*border-radius:\s*9px;/, '页首和页尾按钮必须与左右翻页按钮同为 28px 正方形和 9px 圆角')
assert.match(sidebarPager, /home-sidebar-pager__text--compact/, '窄侧栏必须提供紧凑页数文案，为四个正方形按钮保留空间')
assert.match(sidebarPager, /@container \(max-width:\s*285px\)[\s\S]*?home-sidebar-pager__text--prefix[\s\S]*?display:\s*none;[\s\S]*?home-sidebar-pager__text--full[\s\S]*?display:\s*none;[\s\S]*?home-sidebar-pager__text--compact[\s\S]*?display:\s*inline;/, '窄侧栏必须切换为紧凑页数文案，不能缩窄滚动按钮')
assert.doesNotMatch(sidebarPager, /@container \(max-width:\s*285px\)[\s\S]*?\.home-sidebar-pager__scroll\s*\{[\s\S]*?width:\s*13px;/, '窄侧栏不得再把页首和页尾按钮压成长方形')
