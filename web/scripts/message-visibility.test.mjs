...
const visibilityModule = await readFile(join(webRoot, 'utils/message-visibility.ts'), 'utf8')
...
for (const state of ['public', 'users', 'contacts', 'private']) {
  ...
  assert.match(visibilityModule, new RegExp(`value:\\s*'${state}'`), `edit dialog must offer ${state} visibility`)
...
assert.match(
  editDialog,
  /const canUpdateVisibility = canChangeVisibility\.value[\s\S]*const visibilityChanged = canUpdateVisibility && nextVisibility !== messageVisibility\(currentMsg\)[\s\S]*!contentChanged && !publishTimeChanged && !visibilityChanged/,
