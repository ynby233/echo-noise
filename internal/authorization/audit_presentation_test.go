...
func TestPresentAuditCoversKnownOperationsAndSafeFallbacks(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "administrator audit retention policy",
			record:    models.AdminAuditLog{Capability: string(CapabilityAuthorizationManage), Module: "audit", Action: "update_retention", TargetType: "admin_audit_config", TargetID: "1", Result: "success", Summary: "updated administrator audit retention policy"},
			operation: "已执行管理员审计（调整保留期限）管理操作", result: "成功", safeSummary: "管理员审计保留期限已更新",
		},
		{
