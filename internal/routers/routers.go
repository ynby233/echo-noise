...
func SetupRouter() *gin.Engine {
	...
	updatesRoutes := authRoutes.Group("/updates")
	updatesRoutes.GET("", middleware.RequireCapability(authorization.CapabilityVersionView), controllers.GetUpdates)
	updatesRoutes.PUT("/channel", middleware.RequireCapability(authorization.CapabilityVersionUpdate), controllers.UpdateFollowChannel)
	updatesRoutes.POST("/tasks", middleware.RequireCapability(authorization.CapabilityVersionUpdate), controllers.UpdateTaskInstallationUnavailable)
	updatesRoutes.GET("/tasks/:id", middleware.RequireCapability(authorization.CapabilityVersionView), controllers.GetUpdateTask)
	updatesRoutes.GET("/executor/credential", middleware.RequireCapability(authorization.CapabilityVersionUpdate), controllers.GetExecutorCredential)
	updatesRoutes.POST("/executor/credential", middleware.RequireCapability(authorization.CapabilityVersionUpdate), controllers.CreateExecutorCredential)
	updatesRoutes.DELETE("/executor/credential", middleware.RequireCapability(authorization.CapabilityVersionUpdate), controllers.RevokeExecutorCredential)
	executorRoutes := api.Group("/updates/executor")
	...
	// 评论更新/删除：作者保留自助权；管理动作按独立能力、真实可见范围与站长内容保护判定。
